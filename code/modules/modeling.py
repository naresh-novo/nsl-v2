...
import pandas as pd
import numpy as np
from sklearn.metrics import classification_report, confusion_matrix, accuracy_score, roc_curve, auc, f1_score, precision_score, recall_score, roc_auc_score
...
class ModelMetric:
    ...
    def model_metrics(self, y_pred: np.ndarray, y_true: np.ndarray, y_prob: np.ndarray,tag):
        ...
        output_dict = {'df_tag':tag,'accuracy_score': ac, 'FPR':np.round(fpr[1],2) , 'TPR':np.round(tpr[1],2), 'AUC': np.round(auc(fpr, tpr), 5), 'Sensitivity':np.round(tp/(tp+fn),2)
        ,'Specificity':np.round(tn/(tn+fp),2), 'KS Stat':ks_2samp(y_pred, y_true), 'precision':pr, 'recall':rec}
